...
const PortfolioGrid = () => {
  return (
    <section id="portfolio" className="relative mx-auto max-w-6xl px-6 py-20">
      <div className="mb-8 flex items-end justify-between">
        <div>
          <h2 className="text-3xl font-semibold tracking-tight text-white md:text-4xl">Portfolio</h2>
          <p className="mt-2 max-w-2xl text-zinc-400">
            Curated highlights from private soirées and yacht gatherings across the Riviera.
          </p>
        </div>
        <div className="hidden text-sm text-zinc-500 md:block">Dark chrome • Emerald accent • Minimalist</div>
      </div>

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3">
        {images.map((img, idx) => (
          <figure key={idx} className="group relative overflow-hidden rounded-xl bg-neutral-900 shadow-sm ring-1 ring-white/5">
            <img
              src={img.src}
              alt={img.alt}
